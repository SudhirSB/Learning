...
const f = (0, io_1.format)({ markQuestions: false });
class List extends Question_1.Question {
    ...
    first() {
        return Question_1.Question.about(`the first of ${this.subject}`, async (actor) => {
            const items = await this.answeredBy(actor);
            if (items.length === 0) {
                throw new errors_1.LogicError(f `Can't retrieve the first item from a list with 0 items: ${items}`);
            }
            return items[0];
        });
    }
    last() {
        return Question_1.Question.about(`the last of ${this.subject}`, async (actor) => {
            const items = await this.answeredBy(actor);
            if (items.length === 0) {
                throw new errors_1.LogicError(f `Can't retrieve the last item from a list with 0 items: ${items}`);
            }
    ...
    get(index) {
        return Question_1.Question.about(`the ${ordinal(index + 1)} of ${this.subject}`, async (actor) => {
            ...
            if (index < 0 || items.length <= index) {
                throw new errors_1.LogicError(`Can't retrieve the ${ordinal(index)} item from a list with ${items.length} items: ` + f `${items}`);
            }
            return items[index];
        });
    }
    async answeredBy(actor) {
        ...
        if (!Array.isArray(collection)) {
            throw new errors_1.LogicError(f `A List has to wrap an Array-compatible object. ${collection} given.`);
        }
        return collection;
    }
    describedAs(subject) {
...
class Where extends Question_1.Question {
    ...
    async answeredBy(actor) {
        try {
            ...
            throw new errors_1.LogicError(f `Couldn't check if ${this.question} of an item of ${this.collection} does ${this.expectation}`, error);
        }
    }
    describedAs(subject) {
        this.subject = subject;
